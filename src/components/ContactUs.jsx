...
function ContactUs() {
  ...
  const onSubmit = async (event) => {
    ...
    formData.append("access_key", "7159cbf0-fc87-4d77-8cc2-b3c7f967a9a0");
    try{
    ...
    return (
        <div id="contact-us" className="flex flex-col items-center gap-7 px-4 sm:px-12 lg:px-24 xl:px-40 pt-30 text-gray-700 dark:text-white">
            <Title title="Reach out to us" desc="From strategy to execution, we craft digital solutions that mvoe your business forward."/>
            ...
            <form onSubmit={onSubmit} className="grid sm:grid-cols-2 gap-3 sm:gap-5 max-w-2xl w-full">
                ...
                <div className="sm:col-span-2">
                    ...
                    <textarea name="message" id="" placeholder="Enter Your Message" className="w-full p-3 text-sm outline-none rounded-lg border border-gray-300 dark:border-gray-600" required/>
                </div>
                <button type="submit" className="w-max flex gap-2 bg-primary text-white text-sm px-10 py-3 rounded-full cursor-pointer hover:scale-103 transition-all">
                    Submit <img src={assets.arrow_icon} className="w-full"/>
                </button>
            </form>
        </div>
    )
